...
import { Plus, Clock, MoreHorizontal, X } from 'lucide-react';

const upcomingEvents = [
  {
    id: 1, title: 'UI/UX Design Masterclass: From Beginner to Pro',
    venue: 'Virtual (Zoom)', time: '6:00 PM – 8:00 PM WAT',
    date: 'April 5, 2026', daysLeft: 5, color: 'bg-blue-600', avatar: 'https://randomuser.me/api/portraits/women/20.jpg',
  },
  {
    id: 2, title: 'Figma for Designers: Hands-on Workshop',
    venue: 'Virtual (Zoom)', time: '4:00 PM – 6:00 PM WAT',
    date: 'April 2, 2026', daysLeft: 2, color: 'bg-green-600', avatar: 'https://randomuser.me/api/portraits/women/21.jpg',
  },
  {
    id: 3, title: 'Product Design & User Research Bootcamp',
    venue: 'VI, Lagos, Nigeria', time: '10:00 AM – 2:00 PM WAT',
    date: 'April 10, 2026', daysLeft: 10, color: 'bg-purple-600', avatar: 'https://randomuser.me/api/portraits/men/22.jpg',
  },
];
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="flex items-center gap-2 bg-primary text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-primary-dark transition-all shadow-md">
            ...
            <div className="mb-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {upcomingEvents.map(ev => (
                  <div key={ev.id} className={`${ev.color} rounded-2xl p-5 text-white relative`}>
                    {/* Avatar + countdown */}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50/50">
                      <th className="text-left px-5 py-3 font-semibold text-gray-500 text-xs">Event Name</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs">Date & Time</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs">Venue</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-500 text-xs">Tickets</th>
                  ...
                  <tbody>
                    {tableEvents.map((ev, i) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-3 text-gray-700 text-xs font-medium">{ev.name}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{ev.date}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs">{ev.venue}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs">{ev.tickets.toLocaleString()}</td>
          ...
          {/* ── REMINDERS PANEL ── */}
          <div className="hidden lg:flex flex-col w-64 bg-white border-l border-gray-100 overflow-y-auto p-5">
            <div className="flex items-center justify-between mb-4">
