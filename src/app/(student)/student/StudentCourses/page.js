...
const courseProgress = [
  { id: 1, title: 'UI/UX Design',          progress: 75,  color: 'bg-primary',    icon: <BookOpen    size={18} className="text-primary"     />, status: 'continue',  completed: false },
  { id: 2, title: 'Frontend Fundamentals', progress: 30,  color: 'bg-green-500',  icon: <RotateCcw   size={18} className="text-green-500"   />, status: 'continue',  completed: false },
  { id: 3, title: 'Database Management',   progress: 100, color: 'bg-green-500',  icon: <CheckCircle2 size={18} className="text-green-500" />, status: 'completed', completed: true  },
  { id: 4, title: 'Digital Marketing',     progress: 50,  color: 'bg-yellow-400', icon: <CheckCircle2 size={18} className="text-blue-500"  />, status: 'continue',  completed: false },
];
  ...
  return (
    <div className="relative w-32 h-32 mx-auto">
      <svg className="-rotate-90" width="128" height="128" viewBox="0 0 128 128">
        ...
        <circle cx="64" cy="64" r={r} fill="none" stroke="#10B981" strokeWidth="12"
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 lg:ml-56 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 sm:p-6 space-y-5">
          ...
          {/* ── Top Stats Row — 1 col mobile, 3 col desktop ── */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
            {[
              { icon: <BookOpen size={22} className="text-gray-400" />,                                               label: 'Total Course Enrolled', value: '8' },
              { icon: <CheckCircle2 size={22} className="text-green-500" />,                                          label: 'Courses Completed',     value: '3' },
              { icon: <div className="w-5 h-5 rounded-full border-[3px] border-primary border-r-transparent animate-spin" />, label: 'Ongoing Courses', value: '5' },
            ].map((stat) => (
              <div key={stat.label} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5">
                <div className="flex items-center gap-3">
                  {stat.icon}
                  <div>
                    <p className="text-xs sm:text-sm text-gray-500">{stat.label}</p>
                    <p className="font-extrabold text-gray-900 text-xl sm:text-2xl leading-none mt-0.5">{stat.value}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid lg:grid-cols-4 gap-5 items-start">
            ...
            <div className="lg:col-span-3 space-y-5">
                ...
                <div className="space-y-5">
                  {courseProgress.map((course) => (
                    <div key={course.id} className="flex items-center gap-3 sm:gap-4">
                      {/* Status icon */}
                      <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 border-2 ${
                        course.completed ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-white'
                      }`}>
                        {course.completed ? <CheckCircle2 size={18} className="text-green-500" /> : course.icon}
                      </div>

                      {/* Title + bar */}
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 text-xs sm:text-sm mb-2 truncate">{course.title}</p>
                      ...
                      {/* Action button */}
                      <button className={`text-xs font-bold px-3 sm:px-5 py-2 rounded-xl transition-all flex-shrink-0 ${
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
                ...
                <Link href="/courses/1" className="flex items-center gap-4 border border-gray-200 rounded-xl p-4 hover:border-primary/40 transition-all group">
                  <div className="flex-1 min-w-0">
