  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 lg:ml-56 flex flex-col">
        ...
        <header className="bg-white border-b border-gray-100 px-4 lg:px-6 py-3 flex items-center justify-between sticky top-0 z-20">
            ...
            <div className="grid grid-cols-7" style={{ minHeight: '480px' }}>
              {cells.map((cell, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`border-r border-b border-gray-50 p-1.5 min-h-24 ${
                    ...
                    <div className="space-y-0.5">
                      {events.slice(0, 3).map((ev, ei) => {
                        ...
                        return (
                          <div
                            key={ei}
                            className={`${style.bg} ${style.text} text-xs px-1.5 py-0.5 rounded font-medium truncate cursor-pointer hover:opacity-80`}
                          >
                            {style.label}
                            {ev.time && <span className="block text-xs opacity-70">{ev.time}</span>}
                          </div>
                        );
                      })}
                      {events.length > 3 && (
                        <div className="text-xs text-gray-400 font-medium px-1">+{events.length - 3} more</div>
                      )}
                    </div>
                  </div>
                );
